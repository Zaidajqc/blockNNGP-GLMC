...
#deprecated...
blockNNGP_IRREG2 = function(case="irregular", loc,  y, X, w.obs , n.partition, num.nb,name.cov){
...
#  data1<- data.frame(y=y,x =X[,2:(dim(X)[2])],w,loc1=loc[,1],loc2=loc[,2])
  if(length(data.original$w)>0){
    data1<- data.frame(y=y,x =X[,2:(dim(X)[2])],w,loc.1=loc[,1], loc.2=loc[,2],blocks)
  }else{
    data1<- data.frame(y=y,x =X[,2:(dim(X)[2])],loc.1=loc[,1], loc.2=loc[,2],blocks)
    }
  data1$idx <- 1:nrow(data1)
  
  datablock <-list(data1=data1, W = W, n= nloc, n.blocks= n.blocks,nb =nb,
                   ind_obs1=ind_obs1,num1=num1,indb=indb,coords.D=coords.D, nu=par.cov)
return(datablock)
}
...
#  w2.pred <- w2.pred[(ind1$ix)]
  ...
  # works for n = 1000 update then...
  # #postng.wcopyy2[s,] <- param.pos[[1]]$latent[4001:5000,] # =postng.w1
  # #pred1 <- param.pos[[s]]$latent[1:n,] = linear predictor
