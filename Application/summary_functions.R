...
if(name.prior=="pc.prior")  {
  if (!is.null(resf$summary.hyperpar) && length(resf$summary.hyperpar) > 0) {
   ...
   if (!is.null(resf$marginals.hyperpar$"Beta for base.copy.y2") && length(resf$summary.hyperpar) > 0) {
     ret <- c(ret, list(hyperparLMC = round(as.matrix(resf$summary.hyperpar[5,]), digits = 3)))
   }   
