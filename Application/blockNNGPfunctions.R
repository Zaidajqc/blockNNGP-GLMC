  ...
  for(i in 1:n.pred){
    ...
    for(s in 1:n.sample){
      ...
      if(family=="gaussian"){
        ...
        y2[s] <- rpois(1,exp(t(XX)%*%postng.beta2 + postng.lambda*w0 + w02) )
      }
    }
...
#' @title NNGP_pred()
#' @description This function computes predictions for NNGP models.
...
NNGP_pred = function(case="NNGP", num.nb, data.est, pred.data,res,n.sample,family="gaussian" ){
  ...
  for(i in 1:n.pred){
    ...
    for(s in 1:n.sample){
      ...
      if(family=="gaussian"){
        y2[s] <- XX2%*%postng.beta2 + postng.lambda*w0 + w02 + postng.tausq2*rnorm(1)
      }else if(family=="poisson"){
        y2[s] <- rpois(1,exp(t(XX2)%*%postng.beta2 + postng.lambda*w0 + w02 ))
      }
    }
    mean.y1   <- c(mean.y1, mean(y0))
    mean.y2   <- c(mean.y2, mean(y2))
