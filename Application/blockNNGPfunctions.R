...
blockNNGP_IRREG = function(case="irregular", data.est , n.blocks, num.nb,name.cov,par.cov=NULL,data.pred){
  ...
  nloc <- dim(loc)[1]
  # project all locations into CRS EPSG:5070 in km
  loc.km <- transformkm(loc)
  
  ds  <-  data.frame(x = loc.km[ ,1], y = loc.km[ ,2])  
  fit <- kmeans(ds, centers = n.blocks)
  ds  <- cbind(ds, cluster = as.factor(fit$cluster))
  ds  <- cbind(ds, x1 = loc[ ,1], y1 = loc[ ,2])
  ...
  plot_ordering <- VoronoiPlotly2(fit, ds, n.sd.x = 3, n.sd.y = 3, print.ggplot = F)
  
  USA1 <- gisco_get_countries(country = c("USA","CANADA"), resolution = 1)
  sf_projectedUSA <- st_transform(USA1, crs = 5070)
  sf_projectedUSA$geometry <- sf_projectedUSA$geometry / 1000
  
  p.ggplot2 <- plot_ordering +
    geom_sf(data = sf_projectedUSA, color = "lightgrey", alpha = 0.3, linewidth = 0.1) +  
    xlim(-3700,4000) + ylim(-200,4300) +
    geom_text(data = ordered_polygons, aes(x, y, label = 1:nrow(ordered_polygons)), size = 5) +
    labs(x = "Longitude",y = "Latitude", size = 4) +
    theme(axis.text.x = element_blank(), axis.text.y = element_blank())
  p.ggplot2
  ggsave(paste(getwd(),"/plot_blocksNEW1.eps",sep=''), plot = p.ggplot2, device = "eps")
 ...
 dist.mat <- rdist(loc.blocks)
  ...
  # Convert locations for estimation to sf object (EPSG 4326 is standard lat/lon)
  loc2        <- data.frame(loc2)
  names(loc2) <- c("long", "lat")
  sf_data0    <- st_as_sf(loc2, coords = c("long", "lat"), crs = 4326)
  
  # project  locations for estimation into CRS EPSG:5070 in km
  sf_projected0            <- st_transform(sf_data0, crs = 5070)
  loc_projectedkm          <- sf_projected0
  loc_projectedkm$geometry <- sf_projected0$geometry / 1000
  
  
  ## mask for precision-blockNNGP
  coords.D 	<- st_distance(loc_projectedkm)
...
# Convert pred.coords  to sf object (EPSG 4326 is standard lat/lon)
 pred.coords         <- data.frame(pred.coords)
 names(pred.coords)  <- c("long", "lat")
 sf_data1            <- st_as_sf(pred.coords, coords = c("long", "lat"), crs = 4326)
 # project  pred.coords  into CRS EPSG:5070 in km
 pred.coords          <- st_transform(sf_data1, crs = 5070)
 pred.coords$geometry <- pred.coords$geometry/1000
 
 # Convert obs.coords to sf object (EPSG 4326 is standard lat/lon)
 obs.coords <- data.frame(long=data.est$loc.1, lat=data.est$loc.2)
 sf_data    <- st_as_sf(obs.coords, coords = c("long", "lat"), crs = 4326)
 # project  obs.coords into CRS EPSG:5070 in km
 obs.coords          <- st_transform(sf_data, crs = 5070)
 obs.coords$geometry <- obs.coords$geometry/1000
  ...
  for(i in 1:n.pred){
    ...
    if(length(blocki)>1){
      obs.D <- st_distance(obs.coords[blocki, ])
    }else{
      obs.D <- 0
    }
    obs.pred.D <- st_distance( rbind(pred.coords[i,], obs.coords[blocki,]))[1,]
