## Functions called by blockNNGPfunctions.R
## @author:  Zaida Quiroz (\email{zquiroz@pucp.edu.pe}).

#' @title meancov_nn()
#' @description function  called by  PrecblockNNGP(), it computes
#' covariance matrices B and F for blocks and block's  neighbors
#' to built the precision matrix. 
...
#' @title PrecblockNNGP()
#' @description function  to compute the precision matrix 
#' of blockNNGP
...
#' @title util.index()
#' @description function  to compute indexes  to define the neighbors
#' of each block
...
#' @title VoronoiPlotly2()
#' @description adapted function from VoronoiPlotly()
#'  to plot the number of blocks

VoronoiPlotly2 <- function(fit,  # Fit object from K-Means
                           ds,  # Data frame containing original data and clusters from K-Means
                           n.sd.x = 1,  # Controls the width of the plot
                           n.sd.y = 1,  # Controls the height of the plot
                           print.ggplot = FALSE,  # Plots a diagnostic chart using ggplot2
  ...
  # Plot Check ------------------------------------------------------------------
  p.ggplot <- ggplot() +
    geom_point(data = ds, aes(x, y, color = block),size=0.5) +
    geom_segment(data = vor.df, aes(x = x1, y = y1, xend = x2, yend = y2)) +
    geom_hline(yintercept = y.min) +
    geom_hline(yintercept = y.max) +
    geom_vline(xintercept = x.min) +
