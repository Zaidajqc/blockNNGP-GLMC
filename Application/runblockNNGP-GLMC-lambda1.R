...
## Description:
##
##    The code performs a Bayesian analysis of Palmi-Perales (2023)  
##    model (in Bayesian inference for multivariate spatial models
##    with INLA, R Journal) using block-NNGP and NNGP through 
...
source("blockNNGPfunctions2.R")
source("blockNNGPrgeneric.R")
source("NNGPrgeneric.R")
source("utils.R")
source("summary_functions2.R")
...
runblockNNGP = function(data.est,data.pred,case,n.blocks,num.nb){
  ...
  f.blockNNGP <- y ~ -1 + Intercept.y1 + Intercept.y2 + 
    ...
    f(base.copy.y2, copy = "spatial.field.y1", fixed = TRUE) +
...
runNNGP = function(data.est,data.pred,case,num.nb){
  ...
  f.NNGP <- y ~ -1 + Intercept.y1 + Intercept.y2 + #Intercept.3 + Intercept.4 +
    long1 + lat1 + lat1.2+ tmax1 +  lat2 + vehic2+ 
